...
impl<'a, T> fmt::Display for SpecsWriter<'a, T> where T: Iterator<Item=&'a TargetSpec> + Clone + 'a {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl FromStr for TargetSpec {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Error> {
    	...
    	let next = iter.next();
    	let mut lexer = TargetLexer { inner: iter, partial: None, next: next };
    	parse(&mut lexer).map_err(|cause| Error::chain(
...
struct TargetLexer<T> where T: Iterator<Item=char> {
	inner: T,
	partial: Option<String>,
	next: Option<char>,
}

impl<T> Iterator for TargetLexer<T> where T: Iterator<Item=char> {
	type Item = Result<TargetToken>;
	fn next(&mut self) -> Option<Result<TargetToken>> {
		loop {
			let next_is_letter = self.next.map(|c|
			...
			if next_is_letter {
				...
				self.partial.as_mut().unwrap().extend(self.next.unwrap().to_lowercase());
...
fn parse<T>(lexer: &mut TargetLexer<T>) -> Result<TargetSpec> where T: Iterator<Item=char> {
	Ok(match lexer.next() {
...
fn parse_wrong<R>(wrong: Option<Result<TargetToken>>) -> Result<R> {
	match wrong {
		...
		Some(Ok(TargetToken::Ident(name))) => Err(Error::new(format!("Unexpected identifier `{}`.", name))),
