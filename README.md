...
The recommended way to install Bender is via the shell installer, which detects your platform and places the `bender` binary in `~/.cargo/bin` (on your `PATH`):
```sh
curl --proto '=https' --tlsv1.2 https://pulp-platform.github.io/bender/init -sSf | sh
```
To install a specific version (e.g., `0.28.0`), append ` -s -- 0.28.0` to that command.  Alternatively, you can manually download a precompiled binary from [our Releases on GitHub][releases].

To build from source, [install Rust][rust-installation] and run:
```sh
